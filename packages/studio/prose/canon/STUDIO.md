...
Studio also sheds the playground's front-door job entirely. Its reader arrives already committed, so there is no quickstart to carry, and it is one screen rather than a site: no router, no reading column, no landing page.
...
**The Node half** packs and watches, and does nothing else: `build()` into the directory the dev server serves, watch the source tree, repack, and say so. It is a Vite plugin, so one hook covers `vite dev` and `vite build` alike — and it is `configResolved` rather than a build hook, because Vite mounts its static middleware only for a public directory that exists when the server is created. A pack that lands later is a pack that is not served.

The watcher is told about the source tree by hand, since it sits outside the Vite root, and the packed output is *not* hidden from it: Vite serves a public file only if it is in the set the watcher maintains, so a repack the watcher cannot see is a repack the client cannot fetch. What keeps the page alive across one is that a file no module graph reaches triggers no reload.
...
- an incompatible one strands what no longer projects, with the `conform::*` diagnostics naming each stranded value.
...
Two edges the loop turns on. A ref that went away under the author (a version directory renamed) leaves the document nothing to land in, so whatever the catalog now holds is seeded instead. And a quill that will not **open** — a plate mid-fix is the state an author reloads through most often — keeps the document as text rather than eating it: the panes go empty, the errors say why, and the next repack that compiles gets it back.
...
A throw is unwrapped rather than reported as one line, because a `QuillmarkError` carries every diagnostic and a broken plate is the case that matters: it is the case `quiver preview` used to be reached for.
...
Studio is private, so it depends on `@quillmark/wasm` rather than peering it, and it launches against `fixtures/`, the workspace's source quiver. Publishing it is a packaging change and not a rewrite, which is a property the browser half maintains rather than one it would acquire: it takes its base URL at **runtime**, off the document's own, and imports nothing workspace-relative. The source path and the output directory are the Node half's alone.
