...
**Absence is the only thing either load recovers from.** A specifier that will not resolve names its install; a `quillkit.config.js` that is not on disk falls through to the artifact. Past that answer the fault is the collection's own — an artifact that will not import or will not instantiate, a config that throws — and each fails the verb under the name of what was loading, carrying what threw as `cause` for the bin to print beneath it. The config is where reading a fault as an absence costs a verdict rather than a message: the gate then renders every quill through an engine the author did not write, and prints a pass table for it. So presence is the filesystem's answer, `ERR_MODULE_NOT_FOUND` being equally what a config raises over a specifier of its own.
