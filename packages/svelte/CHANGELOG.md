...
**The `@quillmark/wasm` peer floor is `>=0.106.0-0`, where a region address may name a property of a declared container.** A plate reading `#data.contact.email` off `data` directly now regions as `main.contact.email` where it regioned as `main.contact`, and `fieldAt` answers the same. Nothing here moves: `boxesForField` already falls through to descendants when a field owns no region of its own, and `elementAddrForFieldPath` takes an index terminal only, so a property path declines there and lands its field through `nearestAddrForFieldPath`. A preview click into such a property therefore lands the field that holds it, which is the address a commit can name.

The release's other break is a shape this package draws nothing for: an enum declaring `variants:` rests as a container rather than a bare string, and the control set dispatches on `type: 'enum'` alone. No control meets it yet, and no quill in the workspace declares one.
