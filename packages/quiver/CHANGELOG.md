...
**The `@quillmark/wasm` peer floor is `>=0.106.0-0`.** Nothing here reads a schema shape or a region address, so the release's breaks land outside this package; the floor moves because a quiver hands out `Quill` handles the consumer's copy of the artifact has to be able to parse. A quill whose two enum variants declare one name differently now fails at `getQuill`, the name being one cell of the container whichever world brings it into play.
