...
Bundle and manifest names carry 32 hex chars, 128 bits, because the width answers a chosen prefix rather than a collision. Where the manifest is trusted and the bundles are not — the seeded deployment, the two arriving from different places by design — the digest in the name is the only thing between a served zip and the quill it claims to be, and a quill is a template the backend executes; grinding padding until SHA-256 lands on a chosen prefix costs hours of one GPU at 48 bits and is out of reach at 128. Store keys are the full 64, because the store is keyed by hash and two distinct fonts sharing a prefix would merge into one entry.
