...
- **Reorder is buttons, not drag**, and only the card stack has it: a curated set of heavyweight blocks earns ↑/↓, a scalar list does not (§"Structure mirrors the schema"). Drag's interaction cost (threshold, drop targets, keyboard and touch parity) buys only what the buttons already do.
