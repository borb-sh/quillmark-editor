...
/**
 * Whether `name` is a name a ref can spell. The loaders ask before they seat a
 * catalog entry: a name outside this charset is one `parseQuillRef` refuses, so
 * seated it would be a quill `quillNames` lists and `getQuill` cannot reach.
 */
