...
An island edit lowers through `islandOps` (§Encode), and the entry's **value semantics** draw two boundaries. An anchor inside a cell does not survive a cell edit: `set` replaces the entry whole, and a cell's marks are inside it — upstream declares that a boundary, not an oversight. And a minted island id is this tier's to produce: the positional `isl-{n}` sequence continued, never a UUID or a clock reading, because the id is part of the document's canonical bytes (quillmark `DOCUMENT_STORAGE.md` §Island-id determinism). Nothing mints one today; every `insert` the codec emits carries an id the projection already held.
