...
- **Input rules**: the markdown shorthands (`**`, `*`, `~~`, `` ` ``, `# `, `- `, `1. `, `> `, `---`, and a ` ``` ` code fence) are PM input rules producing ordinary transactions; markdown is an input *shorthand*, never the stored form. No table-entry rule: a pipe row is not a prefix a rule can fire on, and a table is entered from the slash menu instead.
- **Insert menu**: `/` at a word boundary raises the block vocabulary over the caret (`codec/slash.ts` owns the model and the picks; `visual/SlashMenu.svelte` draws it). Unlike the ONE shell-owned format popover, it is the LEAF's surface: a trigger belongs to one caret in one leaf, so `ProseField` mounts it beside the view it reads from. Its keys are the leaf's keymap, innermost of the chain, so the caret never leaves the contenteditable the insert is measured against. `- ` / `1. ` are the only way to START a list: there is no toggle command and no toolbar affordance, so they normalize a heading they wrap, and `# ` declines inside an item: `list_item` is `block+`, making `list_item > heading` representable and unrenderable.
