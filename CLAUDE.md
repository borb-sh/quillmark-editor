...
One load is committed on that, `packages/quillkit/src/__tests__/deploy.browser.test.ts`, because the author's loop cannot exhibit what it holds: a dev server tree-shakes nothing and serves at a root, so a prune that empties a bundle and a base that only resolves at a root are both invisible where the work is done. It opens a site the bin laid, served two segments deep, at each side of the preset's threshold. The browser is whatever the host has (`QUILLKIT_CHROME` overrides; the helper names what it tried).
