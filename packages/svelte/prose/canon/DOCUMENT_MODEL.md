...
**Moving the pin reads upstream.** The break record for a version step is quillmark `docs/migrations/`, one guide per step; a jump across several reads the span guide `0.92-to-0.102.md`, which leads with the breaks a type checker does not report. At this pin one such break is left: the `insertCard(card, at?)` reorder, reported only where the call site is typed, which throws on first use rather than corrupting a document. The gate closes the other — `init` hands out what it instantiates, so a missing await has no name to call. The wasm pin is not the whole of an upgrade: `@quillmark/quiver` versions independently and keeps its own changelog.
...
The session/paint surface (`Engine.open`, `LiveSession`, `PaintOptions` / `PaintResult` / `PageSize`, `ChangeSet`, `supportsCanvas`) is stable at the pinned 0.103.0 and the editor's Preview is its first production consumer: the editor pins the version and rides it.
