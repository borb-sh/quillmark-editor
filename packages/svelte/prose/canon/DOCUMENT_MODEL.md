...
The session/paint surface (`Engine.open`, `LiveSession`, `PaintOptions` / `PaintResult` / `PageSize`, `ChangeSet`, `supportsCanvas`) is stable at the pinned 0.101.0 and the editor's Preview is its first production consumer: the editor pins the version and rides it.
