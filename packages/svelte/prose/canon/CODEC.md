...
**A header toggle is unexpressible**, and that is a chrome consequence, not a guard: `header` is a separate field from `rows`, and `header: []` is not a table. So the header carries no grip at all, offers no row section in the menu a press in it raises, and is therefore never the selected line: there is nothing to move it above, and nothing to delete it into. The last column is the mirror case and the model keeps it too, so a delete gesture there CLEARS its cells instead: a selection has no item to withhold, so it takes the only other reading of "remove this" the shape allows.
...
**The chrome is a band and a menu, and the band occupies no layout.** Every band control is absolutely positioned inside a real data cell or against the grid's frame, hanging into a band the scroller carries as padding: a column's grip lives in its own header cell, a row's in that row's first cell. A grip therefore lines up with the line it names by CONSTRUCTION, being inside it, and tracks wrap, zoom, a font-size change and a horizontal scroll with nothing measured and no rect crossing a state channel, while the grid itself holds the box a paragraph would. Gutter cells buy that same alignment and charge the table's shape for it: an empty row and an empty column in the accessibility tree, so a 3x3 table reads as 4x4 with its headers at column 2. The band is one object and arrives on one trip — hover, focus, or a held subject brings up every grip, the corner and both caps together — because what it says is that this table has lines you can take hold of, which is a claim about the table; a lone grip fading in under the pointer answers one line at a time and answers a question nobody asked.

**A grip carries three gestures at one target, ordered by what each commits to.** A press SELECTS its line, and every verb is then the selection's: Backspace deletes it, Alt+arrow moves it, an arrow steps to the next, Escape goes up to the island, Space raises the menu. A press on the line ALREADY selected raises that menu, which is the pointer's route to the same place and the reason the control needs no second affordance beside it. A press that travels DRAGS, told from the click by a dead zone. Nothing is behind a mode: the first press is always the cheapest reading of a press on a line, and the two rungs above it each need the one below to have happened first. Growth has the one control the band adds: a `+` cap at the far end of each axis, appending being the op a table wants most; every interior insert is a row of the menu instead of a seam per boundary, which is 2n+2 hidden targets to say what four labels say.

**The menu is where an op is spelled out, and it is the codec's own DOM.** It is raised three ways over one set of builders — a second press on a grip, a secondary press on a grip, a secondary press in a cell — and a cell's is both of its lines at once, the column section then the row, which is the menu a table is read from rather than aimed at. A header cell offers no row section, for the reason below. A row that would act on nothing (move left at the first column) is DISABLED rather than withheld, so the menu's shape does not shift with its subject; the row that removes the last column says `Clear column`, because that is what the model will do. Alignment is a radio set under its own heading, the four values the content carries with the live one marked: the group's word carries the instruction, so its rows name a value rather than repeating a verb four times. The menu is `position: fixed`, measured once at open, and it does not TRACK what it opened against — a scroll, a resize or a press outside CLOSES it. That is how it keeps the surface's live-anchor rule (VISUAL_EDITOR §Chrome) rather than breaking it: the rule forbids holding a rect across a reflow, and a menu that is gone by then holds none.

The codec still reports nothing to the shell: the leaf passes no island channel, because a row index inside one island is not a coordinate its surface speaks, and the menu is drawn where the chrome is, inside the leaf.
...
- **island edits are ops, not an overwrite.** The `islandOps` channel (`@quillmark/wasm` 0.102.0) reaches an island's payload and places a new slot, so a table edit keeps every identity anchor in the field. `lower` diffs the island sets and emits it; the overwrite fallback has no island case.
