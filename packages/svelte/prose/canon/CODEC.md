...
**One selection, a rectangle of cells, and the verb over it reads its EXTENT.** A grip names the rectangle covering its line; a press that travels out of the cell it started in sweeps the block it crossed. They are one state, because what Backspace means is decided by the rectangle and not by which gesture drew it: a rectangle spanning every row is a set of COLUMNS and deletes them, one spanning every column is a set of ROWS and deletes those, and anything else is a block of cells and is CLEARED. A row swept cell by cell therefore deletes exactly as the row its grip named does, and the two long-standing exceptions stop being exceptions: the header row alone and the last column both span one axis and both spans, so both clear, which is what the model was always going to do with them. So does the whole table, for the same reason: `header: []` is not a table and the rectangle floors at one column, so "remove every line" has no reading the model can honour. Deleting the TABLE is the island selection's, which the corner and Escape reach.
