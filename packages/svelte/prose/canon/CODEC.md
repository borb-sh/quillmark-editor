...
**A cell is never removed on its own.** Every selected cell goes one of two ways: out with its whole rank, or emptied where it stands. That is the model's shape rather than a policy over it — a rectangle with a hole in it is not a table — and it settles the two cases the extent alone does not. A rank the model REFUSES falls back to the clear rather than to nothing: the header lives in `header` and not in `rows`, so a wide selection reaching row 0 deletes the body rows under it and empties the header's own cells. The rectangle spanning BOTH axes is the whole table, which is no rank deletion at all, since one leaves a table with fewer ranks; it clears, and the last column and the header row alone clear with it. Deleting the TABLE is the island selection's, which the corner and Escape reach.
