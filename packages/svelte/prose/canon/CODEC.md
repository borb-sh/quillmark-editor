...
**A header toggle is unexpressible**, and that is a chrome consequence, not a guard: `header` is a separate field from `rows`, and `header: []` is not a table. So the header carries no grip at all, and is therefore never the line a grip names: there is nothing to move it above, and nothing to delete it into. The last column is the mirror case and the model keeps it too. Both still SELECT, by sweep — a rectangle can cover either — and both then clear, which is the extent rule above rather than a guard beside it: a selection has no item to withhold, so it takes the only other reading of "remove this" the shape allows.
...
**The chrome is a band and a selection, and the band occupies no layout.** Every control is absolutely positioned out of the grid, hanging into a band the scroller carries as padding: a column's grip lives in its own header cell and a row's in that row's first cell, so a grip lines up with the line it names by CONSTRUCTION, being inside it. The two add bars and the corner hang off the grid's frame instead, being about an axis rather than a line. Nothing measures and no rect crosses a state channel, so the band tracks wrap, zoom, a font-size change and a horizontal scroll for free, while the grid itself holds the box a paragraph would. Gutter cells buy that same alignment and charge the table's shape for it: an empty row and an empty column in the accessibility tree, so a 3x3 table reads as 4x4 with its headers at column 2. The band is one object and arrives on one trip: hover, focus, or a held selection brings up every grip, the corner and both bars together. What a band says is that this table has lines you can take hold of, which is a claim about the TABLE; a per-line ladder makes it a claim about whichever line the pointer is over, which is a question nobody asked.

**Six gestures, and the chrome raises nothing.** A grip press SELECTS its line and a grip drag MOVES it, told apart by a dead zone; an add bar appends along its axis. That is the whole control surface, and it is what the band draws: a grip along each line's leading edge, an add bar along each axis's trailing edge spanning it, and the corner where the two grip bands meet. A bar rather than a button at the end of the band, because what an add appends to is the AXIS and not a line — there is no line out past the last one to sit against, and the whole edge is the easiest target on the island to hit. Insertion in the MIDDLE has no control: a row opens where the caret is (Enter at the last row, Tab past the last cell) or at the end and moves, which is two gestures the surface already carries instead of a third it would have to raise a menu for. Alignment round-trips in the content and no gesture reaches it.

**One selection, a rectangle of cells, and the verb over it reads its EXTENT.** A grip names the rectangle covering its line; a press that travels out of the cell it started in sweeps the block it crossed. They are one state, because what Backspace means is decided by the rectangle and not by which gesture drew it: a rectangle spanning every row covers whole COLUMNS and deletes them, one spanning every column covers whole ROWS and deletes those, and anything else covers no rank at all and is CLEARED. A row swept cell by cell therefore deletes exactly as the row its grip named does.

**A cell is never removed on its own.** Every selected cell goes one of two ways and there is no third: out with its whole rank, or emptied where it stands. That is the model's shape rather than a policy over it — a rectangle with a hole in it is not a table — and it is what decides the two cases where the extent alone does not. A rank the model REFUSES falls back to the clear rather than to nothing: the header lives in `header` and not in `rows`, so a wide selection reaching row 0 deletes the body rows under it and empties the header's own cells, where leaving them holding their old text would be the third outcome. And the rectangle spanning BOTH axes is the whole table, which is not a rank deletion at all, since a rank deletion leaves a table with fewer ranks; so it clears, and the last column and the header row alone clear with it. Deleting the TABLE is the island selection's, which the corner and Escape reach.
