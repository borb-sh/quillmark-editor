...
**And the bundle names the rest of what it carries.** A browser resolves nothing, so both siblings are compiled in too, and no manifest in a consumer's tree records which copies. The build stamps all three (`scripts/carried.mjs`): `__CARRIED__` for the running client, `dist/client/carried.json` for a consumer reading the tarball without running it, and one line in the release notes. Each sibling's stamp is its manifest version qualified against its own release tag: `0.1.0` claims the bytes are the released ones, and `0.1.0+3.a1b2c3d`, `0.0.0+untagged.a1b2c3d` and `0.1.0+nogit` are what keep that claim honest. A label rather than a refusal: nobody installs the contents of a bundle, so the coordinate is for diagnosis, and a release whose notes name unreleased sibling code reads as wrong.
