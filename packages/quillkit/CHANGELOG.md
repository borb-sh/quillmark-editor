...
**The carried `@quillmark/wasm` is 0.113.0, where a `form-field` widget's rect is the box it prints in.** The helper reported the rect from beside the box, so an inline widget regioned one box-height low and a centred or right-aligned one regioned at the left margin. A quill under test is affected where it declares a widget: the regions `quillkit check` reads and the AcroForm `/Rect` it writes land on the widget now, so a golden held over a stamped PDF or over a region table recomputes once, and a plate that compensated for the offset shifts by it. Beside it, a `pdfform` quill with overlapping widgets answers `fieldAt` with the last stamped rather than the first, an image in a `richtext` field resolves a quill asset where it failed the compile outright, and a U+2028 or U+2029 in document text becomes a space where it opened a spurious block marker. The document API moves under a host, not under this bin: `resolve` lives on the reader and the storage DTO verbs are `toStored` / `fromStored` / `tryFromStored` / `loadStored`.

**The previous carried step, 0.112.0, is where every member of the content vocabulary spells its payload in `attrs`.** A quill under test is unaffected: the respelling is a wire shape, `quillkit check` reads diagnostics rather than content, and a quill's own markdown and Typst are untouched. What moves is a golden held over rendered bytes — coincident marks reorder on the new tie-break and the generated Typst nests their wraps the other way for identical glyphs — so a content hash or a Typst golden recomputes once.
