...
**Moving the pin reads upstream.** The break record for a version step is quillmark `docs/migrations/`, one guide per step; a jump across several reads the span guide `0.92-to-0.102.md`, which leads with the breaks a type checker does not report. At this pin two such breaks are left. The `insertCard(card, at?)` reorder is reported only where the call site is typed, and throws on first use rather than corrupting a document. The 0.104.0 index respelling is reported nowhere at all: `FieldRegion.field` and `ContentHit.field` are still `string`, so a prefix match or a trailing-digit read written against the dotted form compiles and silently stops matching — which is why the two places this package read one carry a case each (`boxesForField`, `elementAddrForFieldPath`). The gate closes the third — `init` hands out what it instantiates, so a missing await has no name to call. The wasm pin is not the whole of an upgrade: `@quillmark/quiver` versions independently and keeps its own changelog.
...
Field addresses are one canonical `DocPath` across the boundary: `Diagnostic.path`, `ContentHit.field`, and `FieldRegion.field` all speak `main.<field>` / `main.body` / `cards.<kind>[<i>].<field>`, cards keyed by **absolute document index** (no per-kind ordinal). An array element is bracketed on every one of them (`main.references[0]`), so a diagnostic path and a geometry address for one place are the same string and join on equality; a nested key stays dotted. The editor routes on `parseDocPath` and builds with `formatDocPath` rather than a hand-rolled string grammar; diagnostics routing is confirmed against the real grammar, not best-effort.

The session/paint surface (`Engine.open`, `LiveSession`, `PaintOptions` / `PaintResult` / `PageSize`, `ChangeSet`, `supportsCanvas`) is stable at the pinned 0.104.0 and the editor's Preview is its first production consumer: the editor pins the version and rides it.
