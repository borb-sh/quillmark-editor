...
## A richtext field is stored in whichever form put it there

`getStored` is **transport, not interpretation**: it returns the payload value verbatim, so its return shape is a fact about how the value was written, not about the field's declared type. A SEEDED document written through the typed writer holds `Content` at a richtext field; a LOADED one (`Document.fromMarkdown` / `fromJson`) holds the markdown string it parsed, until that field is next written. A body differs from both and is always `Content`. All three are the boundary's design; the binding's own docstring is what overstates it, promising "a content object for a richtext field" where the implementation is a verbatim payload read.

So the prose leaf reads a string through the boundary's own markdown door (`importMarkdown`), which is the same projection the codec exports back out. It is not a workaround around a bug; it is the codec meeting a two-form read. It is load-bearing either way: without it the decode crashed on every richtext field of every loaded document — the form the editor never meets while a document is only ever seeded, and the first form a real consumer meets. What would delete it is a CONTENT-typed read at the schema plane; the one that exists (`quill.reader(doc).get`) projects a richtext field to markdown, so every codec consumer imports it back itself.
