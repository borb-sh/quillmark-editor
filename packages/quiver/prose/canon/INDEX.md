# Quiver Canon Index

Canonical documentation for `@quillmark/quiver`: collections of quills, resolved and loaded.

- [QUIVER.md](QUIVER.md): the distribution model, the loaders, `getQuill`, the render boundary, and the author-side harnesses.

The consumer-facing walkthrough with runnable snippets is the package's [`README.md`](../../README.md). The rules spanning the packages are the [workspace's](../../../../prose/canon/INDEX.md).

Work that is not settled lives in GitHub issues.
