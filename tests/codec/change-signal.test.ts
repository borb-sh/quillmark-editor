...
import type { Addr, Document } from '$lib/core';
import { quill } from './_util.js';
...
describe('the prose leaf change signal', () => {
	...
	it('fires onChange on the FIRST edit to an unset field, which installs', () => {
		// `tag_line` is declared with a `default:` and no stored value: the first
		// edit goes down the install branch rather than applyChange, and the host's
