...
**Four mutable pointers carry the whole lag**: `index.html`, the `404.html` copy of it, `_app/version.json`, `quiver/latest.json`. A reload sees the new build within ten minutes of the deploy finishing: browser freshness is `max-age` less `Age`, so edge and tab share one window rather than stacking two. SvelteKit fetches `version.json` `no-cache`, so the recovery check below bypasses the browser cache.

Ten minutes is the contract for a page load, not for a tab. No service worker, no `kit.version.pollInterval`, nothing pushed: a tab keeps the chrome it loaded until something reloads it. SvelteKit consults `updated` only on the recovery path below, so `pollInterval` would set a store and nothing more.

One case self-heals: a client-side navigation into a route chunk the redeploy removed rejects on import, SvelteKit compares `version.json` against the version baked into the bundle, and a mismatch turns the navigation into a full page load. It reaches only a chunk the tab has not already imported, so across two routes a tab that opened both holds every chunk in memory and never runs the import that would notice.
