...
describe('the cell codec: a cell is its own content unit', () => {
	...
	it('an anchor in a cell is preserved verbatim across an edit', () => {
		const c: TableCell = {
			text: 'abcd',
			marks: [{ start: 3, end: 3, type: 'anchor', attrs: { id: 'a1' } }]
		};
		...
		const anchor = next.marks.find(isAnchorMark)!;
	...
	it('a cell anchor holds its position against an insertion at it, as a field’s does', () => {
		const c: TableCell = {
			text: 'abcd',
			marks: [{ start: 2, end: 2, type: 'anchor', attrs: { id: 'a1' } }]
		};
		...
		const anchor = cellFromDoc(edited, c).marks.find(isAnchorMark)!;
	...
	it('two spellings of one mark are one cell, whatever order the keys arrive in', () => {
		...
		const stored: TableCell = {
			text: 'linked',
			marks: [{ start: 0, end: 6, type: 'link', attrs: { url: 'https://x.com' } }]
		};
		const projected: TableCell = {
			text: 'linked',
			marks: [{ attrs: { url: 'https://x.com' }, type: 'link', end: 6, start: 0 }]
		};
