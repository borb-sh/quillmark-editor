...
**Click-to-NodeSelect belongs to an atom with no interior.** An image is one: there is nothing to enter, so a press on it is unambiguous and it keeps the click. A table is not: it has cells, and every point on it is nearer to some cell than to the intent "select the whole thing". So a pointer press resolves to a CARET everywhere but on a handle, and the island is three bands with three owners. INSIDE the frame (cell padding, the borders between cells) is the table's: a press focuses the nearest cell, measured against the mounted hosts' rects rather than against the last cell focused, since one gesture with two outcomes depending on history is a surface that feels haunted. OUTSIDE it (the island's own padding, the space beside the grid) is the DOCUMENT's: a press means "write here, beside the table" and lands a gap cursor, or the neighbouring block's own edge where the document holds a text position there (`prosemirror-gapcursor`, which the block-schema plugin stack mounts). A HANDLE is the third, and what it selects is a LINE, never the node. Pulling a press into a cell from outside would be the same violation at smaller scale: a caret arriving where the pointer did not aim. The guard is a `mousedown` listener on the NodeView's `dom`, because that is where PM arms the selection its mouseup takes; `stopEvent` gates the whole subtree, so widening it to cover a press would take PM's keydown routing with it.
...
**The chrome occupies no layout, and it raises no menu.** Every control is absolutely positioned inside a real data cell, hanging into a band the scroller carries as padding: a column's handle lives in its own header cell, a row's in that row's first cell. A handle therefore lines up with the line it names by CONSTRUCTION, being inside it, and tracks wrap, zoom, a font-size change and a horizontal scroll with nothing measured and no rect crossing a state channel, while the grid itself holds the box a paragraph would. Gutter cells buy that same alignment and charge the table's shape for it: an empty row and an empty column in the accessibility tree, so a 3x3 table reads as 4x4 with its headers at column 2. A press on a handle SELECTS its line, and every verb is then the selection's: Backspace deletes it, Alt+arrow and a drag move it, an arrow steps to the next, Escape goes up to the island. Insertion is a seam at each boundary, the leading and trailing ones included, so an append and an interior insert are one gesture at different places. Alignment is the one op with no gesture and so the one surface still raised: four glyphs on the column's own selection. The codec reports nothing to the shell: the leaf passes no island channel, because a row index inside one island is not a coordinate its surface speaks.
