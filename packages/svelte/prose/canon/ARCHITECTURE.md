...
Its routes, and the host scale they share, are [PLAYGROUND.md](../../../playground/prose/canon/PLAYGROUND.md). One of them is architectural: `/playground` is the reference **split-pane shell**, where the caret bridge is consumer-layer glue: the editor emits places and carets, the preview surfaces hits, and the shell joins them, so neither surface imports the other. Both hops are pass-throughs (`onCaretMove={preview.focusPosition}`, `onCaretPick` → `setCaret`) because the two surfaces share one address vocabulary, declared once in `/core` (`Place`, `DocPath`) rather than structurally per subpath. What that route demonstrates about the mounting site is how little there is of it: each pane states a track that may shrink below its content, the editor takes `.qm-pane`, and the column is the surface's own.
