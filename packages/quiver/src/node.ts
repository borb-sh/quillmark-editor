...
Quiver.fromBuiltDir = async function fromBuiltDir(dirPath: string): Promise<Base> {
	return loadBuiltQuiver(new FsBuiltTransport(dirPath));
};

Quiver.fromPackage = async function fromPackage(specifier: string): Promise<Base> {
