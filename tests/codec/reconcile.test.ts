...
// re-hydrates only when the stored corpus diverges from what the codec last knew;
...
import { createReconciler, corpusEqual } from '$lib/core/codec';
import { md } from './_util.js';

describe('reconciliation gate', () => {
	it('no re-hydrate when the corpus matches the last-known', () => {
	...
	it('re-hydrate when the corpus diverges (a foreign edit)', () => {
	...
	it('corpusEqual is key-order-insensitive and mark-aware', () => {
		expect(corpusEqual(md('**bold** text'), md('**bold** text'))).toBe(true);
		expect(corpusEqual(md('**bold** text'), md('*em* text'))).toBe(false);
	});
});
