...
export async function renderQuiverSamples(
	...
	for (const name of quiver.quillNames()) {
		for (const version of quiver.versionsOf(name)) {
			...
			results.push(await renderOne(quiver, name, version, outDir, opts.engine, format));
		}
	}
...
function isSelected(name: string, ref: string, opts: RenderQuiverSamplesOptions): boolean {
...
async function renderOne(
	...
	try {
		...
		try {
			result = await engine.render(quill, doc, format ? { format } : undefined);
		} finally {
			doc.free();
		}
	} catch (err) {
...
function printSummary(quiverName: string, outDir: string, results: RenderedSample[]): void {
	const count = (s: RenderedSample['status']) => results.filter((r) => r.status === s).length;
...
function renderIndexHtml(quiverName: string, results: RenderedSample[]): string {
	const cards = results
		.map((r) => {
			const body =
				r.status === 'rendered'
