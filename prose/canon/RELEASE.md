...
Quiver is published and settled; `ui` is `0.0.0` and being rewritten. Lockstep would drag quiver's version through churn its consumers do not care about, so each package carries its own version and a change to one leaves the other's alone. The packages have no edge between them ([DEPENDENCIES.md](DEPENDENCIES.md)), so there is no shared substrate co-evolving across them to argue the other way; that argument holds *within* `ui`, which is why its surfaces are subpaths of one package rather than packages of their own.
