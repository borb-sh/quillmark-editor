...
export function createPaintLoop(
	...
	function makeSlot(page: number): PageSlot {
		...
		// THEMING permits to be plain `--qm-bg` — the same value. A hairline holds
		// at both poles where a shadow's offset does not (SURFACES §Elevation). It
		// is the boundary of the paper, not a mark on it, so it leaves the overlay's
		// no-ink-at-rest rule untouched (PREVIEW §Overlay). `border-box`, so the
		// stroke lands inside the width the container gives and `clientWidth` below
		// still measures the paper.
