...
**Not** a document-model design: the `Document`, its mutators, the WASM boundary, and diagnostics are quillmark's, each with a canonical home in quillmark canon. This ledger pins the *exact* quillmark surface `@quillmark/editor` V1 consumes, cites where each is documented, and marks its stability. It is the one place the version coupling to `@quillmark/wasm` is recorded; when a surface below moves, the editor's dependency moves with it. `src/lib/core/` re-exports the whole surface verbatim: the one door the rest of the package crosses to reach the WASM package.
...
## A loaded document reads back a string where the contract says content

`getStored` documents its own return as "the raw payload value of a field (**a content object for a richtext field**, a scalar/array/object otherwise)", and `Card.body` as "**always this content shape on read, never a markdown string**". A document that was LOADED (`Document.fromMarkdown` / `fromJson`) honours the second and not the first: its body reads back as `Content`, and every richtext FIELD reads back as the markdown string it parsed, until that field is next written. A SEEDED document written through the typed writer reads back `Content` at the same address. The schema plane is consistent either way (`reader.get` is markdown on both), so the deviation is the verbatim read alone, and it is upstream's: the write path already normalizes, since an `applyChange` against a string-stored field succeeds and leaves `Content` behind.

Until it moves, the prose leaf reads a string through the boundary's own markdown door (`importMarkdown`), the same projection the codec exports back out. That workaround is load-bearing: without it the decode crashed on every richtext field of every loaded document — the form the editor never meets while a document is only ever seeded, and the first form a real consumer meets. It is a workaround, not a model: when `getStored` reads `Content` for a parsed document, the branch goes.
...
- **`ContentIsland.props`**: the union pins `TableProps` for `table` and `ImageProps` for `image` (`TableCell` the cell shape); an island of any other type round-trips with opaque `props`. The codec needs no hand-rolled `IslandTable*` / `IslandImage*` shapes or guards ([CODEC.md](CODEC.md) §Islands).
