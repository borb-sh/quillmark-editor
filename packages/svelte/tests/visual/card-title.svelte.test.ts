...
// that stays mounted. The next bump rebuilds the header from the document, where
// the field now rests as `Content`, and reads it by its text.
//
// On its own quill: the reference quill's one templated title (`section`'s
// `{heading}`) names a `string`, which commits through the typed writer and a bump
// and never rests as `Content`. Mounted at `VisualEditorInner`, which holds
// `getActiveLeaf`: jsdom drives no contenteditable, so the commit is a transaction
// dispatched into the leaf's own view, as `codec/plaintext-leaf` drives one.
...
// jsdom implements none of these, and a focus into a leaf reaches all four: the
// reveal's hop, the arrival wash, and the caret rect PM measures to scroll to.
...
describe('a {field} title over a prose field', () => {
	it('follows the leaf commit without a bump, and survives the next one', async () => {
		...
		// A bump rebuilds the header from the document: read by its text, not
		// stringified.
