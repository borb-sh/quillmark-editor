...
**V1 builds on `@quillmark/wasm` 0.100.0**, one release past the 1.0 API freeze. Every verb in the table below is stable at that pin. The settled ground it stands on: `store*` verbs for verbatim writes, one unified `Addr` (`{card?, field?}`), the schema-bound `writer`/`reader` doors (`quill.writer(doc)` / `quill.reader(doc)`), a card-first `insertCard(card, at?)`, the quill-free transport read `getStored` (distinct from `reader.get` and `reader.getContent`), the `quill.resolve(doc)` value view, `code`-bearing mutator diagnostics, and the canonized anchor-id policy (caller-supplied, unique, invariant) that lets the editor mint anchors at a selection ([CODEC.md](CODEC.md) §Marks).
...
At the 0.100.0 target the whole table is stable API and no typing gap is open: `ContentLineKind` is on the public entry point, so the codec builds its `setKind` op typed (`encode.ts`, `kindOp`).
...
The session/paint surface (`Engine.open`, `LiveSession`, `PaintOptions` / `PaintResult` / `PageSize`, `ChangeSet`, `supportsCanvas`) is stable at the pinned 0.100.0 and the editor's Preview is its first production consumer: the editor pins the version and rides it.
