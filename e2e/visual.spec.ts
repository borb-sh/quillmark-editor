...
test.describe('visual editor', () => {
	...
	test('(s1) a label click lands focus in the control it names', async ({ page }) => {
	...
	test('(s2) a label click reaches the controls `for` cannot', async ({ page }) => {
	...
	test('(s3) the control takes its name from the label, and carries no second one', async ({
		page
	}) => {
		await reveal(page, 'main-letterhead_title');
	...
	test('(s4) the required marker names the control along with the label text', async ({ page }) => {
	...
	test('(s5) guidance is a themed surface on hover, and describes the control while closed', async ({
		page
	}) => {
		await reveal(page, 'main-letterhead_title');
	...
	test('(s6) keyboard focus raises the guidance; Escape dismisses it', async ({ page }) => {
		await reveal(page, 'main-letterhead_title');
		const popover = page.getByTestId('hint-main-letterhead_title-popover');
