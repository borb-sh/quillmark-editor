...
**An address's boxes are `fieldBoxes`'s union, or its raw `regions()` rects where there is none** (`geometry.ts`, `boxesForField`; the one rule the overlay and the scroll both read). `fieldBoxes` is span-bearing-content-only: it answers `[]` for a scalar the plate places without tracking its content and for a `richtext[]` element, and the boundary says what such a field's box is — a single `regions()` rect. *Under* the address is the same sentence one granularity down: an array's own ink is its elements' (`main.references` is named by no region, only `main.references.0` and `.1` are), so a host holding the declared path reaches the rows it prints. Without the fallback the overlay drew the content half of a compile and nothing else — on the reference memo, two addresses of the six the compile names.
...
**An absent `pos` IS the placement rung**, so the payload widens rather than a second hook appearing (`Landing`, `/core`). A fabricated `0` would be an invented offset wearing a real one's type; no `precision` enum either, since `granularity` already carries cluster/segment and a second axis derived from the first is drift with nothing to catch it. `granularity` decides precision where there is a `pos`: `'cluster'` → caret-exact; `'segment'` → focus the field/segment, no exact caret.

**There is no third rung hit-testing `regions()` by hand.** Its rects are bounding boxes over ink the field does not fill — `main.body`'s union spans the gap between two disjoint segments — so a click in that gap would land on the body by geometry the compile never claimed. What the two queries answer is the compile's own answer, and where neither does the hook stays silent.
