...
**Click-to-NodeSelect belongs to an atom with no interior.** An image is one: there is nothing to enter, so a press on it is unambiguous and it keeps the click. A table is not: it has cells, and every point on it is nearer to some cell than to the intent "select the whole thing". So a pointer press on or around one resolves to a CARET, always, and the island's chrome is two bands with two owners. INSIDE the frame (gutter, corner, cell padding, the borders between cells) is the table's: a press focuses the nearest cell, measured against the mounted hosts' rects rather than against the last cell focused, since one gesture with two outcomes depending on history is a surface that feels haunted. OUTSIDE it (the island's own padding, the space beside the grid) is the DOCUMENT's: a press means "write here, beside the table" and lands a gap cursor, or the neighbouring block's own edge where the document holds a text position there (`prosemirror-gapcursor`, which the block-schema plugin stack mounts). Pulling that press into a cell would be the same violation at smaller scale: a caret arriving where the pointer did not aim. The guard is a `mousedown` listener on the NodeView's `dom`, because that is where PM arms the selection its mouseup takes; `stopEvent` gates the whole subtree, so widening it to cover a press would take PM's keydown routing with it.
...
**A header toggle is unexpressible**, and that is a chrome consequence, not a guard: `header` is a separate field from `rows`, and `header: []` is not a table. So the header carries no handle at all: what its menu could hold is one item, and "insert below the header" is what the first body row's "insert above" already is. The last column offers no delete for the same reason, and in both places the item is ABSENT rather than disabled.
