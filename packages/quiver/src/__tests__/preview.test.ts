...
import { renderQuiverSamples, type RenderQuiverSamplesOptions } from '../preview.js';
...
function makeEngine(): RenderQuiverSamplesOptions['engine'] {
	return {
		async render(_quill: unknown, _doc: unknown, opts: unknown) {
			...
			return {
				artifacts: [{ format, bytes: new TextEncoder().encode(`rendered:${format}`) }],
...
function explodingEngine(render: () => never): RenderQuiverSamplesOptions['engine'] {
...
describe('renderQuiverSamples', () => {
	...
	it('records a render failure without aborting the run', async () => {
		...
		expect(results.find((r) => r.ref === 'memo@1.0.0')!.reasons).toEqual(['boom']);
		expect(results.find((r) => r.ref === 'plain@1.0.0')!.reasons).toEqual(['boom']);
	});
