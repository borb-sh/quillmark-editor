...
**The chrome is a band and a selection, and the band occupies no layout.** Every control is absolutely positioned out of the grid, hanging into a band the scroller carries as padding: a column's grip lives in its own header cell and a row's in that row's first cell, so a grip lines up with the line it names by CONSTRUCTION, being inside it. The two add bars hang off the grid's frame instead, being about an axis rather than a line. Nothing measures and no rect crosses a state channel, so the band tracks wrap, zoom, a font-size change and a horizontal scroll for free, while the grid itself holds the box a paragraph would. Gutter cells buy that same alignment and charge the table's shape for it: an empty row and an empty column in the accessibility tree, so a 3x3 table reads as 4x4 with its headers at column 2. Each control comes up alone: a grip or a bar is drawn while the pointer is on THAT control, while it holds the focus, or while it is what a gesture is about — the selected line's grip, the lifted one. A pointer crossing a table says nothing about which line it is crossing, so a band that arrived whole would put six controls up to answer a question the pointer has picked out none of. The targets are there throughout either way, an invisible button being hit-tested like any other, so what the reveal decides is what the table LOOKS like under a passing pointer rather than what it answers to.
