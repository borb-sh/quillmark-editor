...
**The carried `@quillmark/wasm` is 0.112.0, and the gate answers for a quill's own declarations.** A variant-bearing enum whose `default:` is not a string is a load error, where it loaded clean and then selected nothing at render; a `$quill` version segment spelling a leading `+` (`memo@+2.+1`) is one too, having parsed as digits it does not write back; and an unquoted `version: 1.0` keeps its fraction, where the loader dropped it and then failed validation naming the `'1.0'` the author had written. Beside them the PDF spine checks what it was trusting: `quillkit check` refuses a pdfform base carrying its own `/AcroForm` (`pdf::existing_acroform`), which shipped a double-form PDF whose behavior was the reader's choice, and a non-finite widget rect (`pdf::bad_rect`), which wrote an unparseable PDF. Both are the quill's to fix — strip the background, correct the `form.json` geometry — and both are PDF output only, SVG and PNG drawing values regardless.

**What a quill renders moves at this pin with no quill changing.** Document text reaches the page as the characters it holds: `3--5`, `wait...` and `-5` set as they were typed, where Typst's lexer shorthands took an en dash, an ellipsis and a minus sign off the page. An array-bound pdfform text widget is multiline, so the interactive PDF stops collapsing to its first line while the flattened raster stacked every one. A quill holding a golden over its own output re-bakes it once.
