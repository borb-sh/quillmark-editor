...
/** One page slot whose box measures `scale`×(612×792) CSS px, so at `scale` 1 a click
 *  at (x, y) reads back as the PDF point `(x, 792 - y)` and the queries can be keyed on
 *  plain numbers. */
...
describe('the click ladder', () => {
	...
	// The slack both queries carry is the pointer's, so it is a fixed size on screen
	// and a varying one in the document: halve the page's drawn width and a CSS
	// pixel covers twice as many points, so the same slack is twice the points. The
	// relation is asserted rather than the constant, which the bridge alone owns.
	it('converts the click slack at the scale the page is drawn', () => {
