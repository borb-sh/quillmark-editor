...
**The `editor` namespace is the write unit, and one verb owns it.** Because `storeExtNamespace` REPLACES the namespace it targets (preserving sibling namespaces, not sibling keys), a writer that stores only its own key destroys the others, silently. Both writers therefore go through `patchEditorExt(doc, addr, patch)` (`ext.ts`), which merges and treats an `undefined` value as a key drop; a consumer seeding the namespace reaches the same verb off `@quillmark/ui/visual`. That is what keeps a dismissal from wiping every renamed card's title, and what makes key N+1 inherit the rule instead of re-deriving it.
...
- **Card-instance identity is a session key**: an in-memory id per card, reordered in lockstep with the content and resolved to an index only at the mutation boundary (`structure.ts` `IdSeq`). The document carries no per-card handle: a key is minted at mount and dies at unmount, so two mounts of one document agree on position and nothing else. Per-card state that must outlive a mount rides the card itself (`$ext.editor`), which a reorder carries along and which therefore needs no handle to find its way back.
