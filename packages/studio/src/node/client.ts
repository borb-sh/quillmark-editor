 ...
 *
 * One package is what makes the two halves agree by construction. The client compiles
 * in where it looks for its quiver (`quiver/` under its own base), and the bin lays one
 * out there; a bin shipped apart from the client would make that agreement a version
 * range to keep true.
 */
 ...
 * imports the source, `bin/` once compiled and in the tarball — and a constant that was
 * right in one would be silently wrong in the others.
 */
